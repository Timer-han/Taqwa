...
QUESTION_SUGGEST_MSG = (
    "Спасибо, что вносишь вклад в базу вопросов. Чтобы добавить вопрос, следуй инструкции ниже:\n"
    "Первым делом сформулируй вопрос и отправь его сюда без вариантов ответа. То есть в таком виде, каким его увидит ученик.\n\n"
    "Пример: ```В какой временной период надо соблюдать пост?```"
)

QUESTION_RECEIVE_MSG = (
    "А теперь добавь варианты ответа через запятую\n\n"
    "Пример: \`\`\`когда проснулся - до вечера, от восхода - до захода, от захода - до восхода, с зенита - до полуночи\`\`\`"
)
